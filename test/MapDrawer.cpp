...
//        if(vpMPs[i]->isBad() || spRefMPs.count(vpMPs[i])) // 不包括ReferenceMapPoints
        if(spRefMPs.count(vpMPs[i])) // 不包括ReferenceMapPoints
            continue;
...
void MapDrawer::DrawMapLines()
{
    ...
    {
//        if(vpMPs[i]->isBad() || spRefMPs.count(vpMPs[i])) // 不包括ReferenceMapPoints
        cv::Mat pos1 = vpMLs[i]->GetWorldPos1();
        cv::Mat pos2 = vpMLs[i]->GetWorldPos2();
...
//    }
//}
void MapDrawer::DrawInitialCamera(pangolin::OpenGlMatrix &Twc)
{

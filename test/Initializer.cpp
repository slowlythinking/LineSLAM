 ...
 */
Initializer::Initializer(const Frame &ReferenceFrame, const vector<cv::KeyPoint> &keypoint1, float sigma, int iterations)
{
    mK = ReferenceFrame.mK.clone();

    mvLineKeys1 = ReferenceFrame.goodlines;
    mvKeys1 = keypoint1;
//    int pointNum = mvLineKeys1.size();
//    mvKeys1.clear();
//    mvKeys1.reserve(2*pointNum);
//    for(int i = 0; i < pointNum; i++)
//    {
//	cv::KeyPoint linekp1(mvLineKeys1[i].startPointX,mvLineKeys1[i].startPointY,0.3);
//	cv::KeyPoint linekp2(mvLineKeys1[i].endPointX,mvLineKeys1[i].endPointY,0.3);
...
//注意vMatches12中存的是线段匹配的结果，即其中是goodlines向量中线段的下标
//keypoint2中存的是当前帧中的线段的点
bool Initializer::Initialize(const Frame &CurrentFrame,  const vector<cv::KeyPoint> &keypoint2, const vector<cv::DMatch> &vMatches12, cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    // Fill structures with current keypoints and matches with reference frame
    // Reference Frame: 1, Current Frame: 2
    // Frame2 特征点
    mvLineKeys2 = CurrentFrame.goodlines;
    mvKeys2 = keypoint2;

    //将当前帧中线段的端点存入mvkeys2
//    int pointNum2 = mvLineKeys2.size();
//    mvKeys2.clear();
//    mvKeys2.reserve(2*pointNum2);
//    for(int i = 0; i < pointNum2; i++)
//    {
//	cv::KeyPoint linekp1(mvLineKeys2[i].startPointX,mvLineKeys2[i].startPointY,0.3);
//	cv::KeyPoint linekp2(mvLineKeys2[i].endPointX,mvLineKeys2[i].endPointY,0.3);
//	mvKeys2.push_back(linekp1);
//	mvKeys2.push_back(linekp2);
//    }

    // mvMatches12记录匹配上的特征点对
    int matchnum = vMatches12.size();
    mvMatches12.clear();
    mvMatches12.reserve(2*matchnum);
    ...
    {
	mvMatches12.push_back(make_pair(2*(vMatches12[i].queryIdx),2*(vMatches12[i].trainIdx)));
	mvMatches12.push_back(make_pair(2*(vMatches12[i].queryIdx)+1,2*(vMatches12[i].trainIdx)+1));
    }
	...
	srand((int)time(0));
        ...
        {
            // 产生0到N-1的随机数
	    int maxtemp = vAvailableIndices.size()-1;
            int randi = rand()%maxtemp;
...
//    if(SH == 0 && SF == 0)
 ...
 //   cout << "多线程计算单应矩阵和本质矩阵完成,其RH评分为：" << RH << endl;
 //   cout << "其SH评分为：" << SH << endl;
    cout << "其RH评分为：" << RH << endl;
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 利用重投影误差为当次RANSAC的结果评分
        currentScore = CheckFundamental(F21i, vbCurrentInliers, mSigma, temppp);
	if(temppp > inliernum)
	    inliernum = temppp;
    ...
    cout << "------------------------------------------------满足距离要求的内点个数为: " << inliernum << endl;
    cout << "------------------------------------------------分数SF为: " << score << endl;
}
 ...
 * - Multiple View Geometry in Computer Vision - model selection 4.7.1 RANSAC
 */
float Initializer::CheckFundamental(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma, int &temppp)
{
    temppp = 0;
...
//    const float thScore = 25.991; // 和CheckHomography的一致
...
//	cout << "chiSquare1: " << chiSquare1 << "--chiSuare2: " << chiSquare2;
...
{
    cout << "开始基础矩阵重建函数" << endl;
    int N=0;
    cout << "vbMatchesInliers.size(): " << vbMatchesInliers.size() << endl;
    for(size_t i=0, iend = vbMatchesInliers.size() ; i<iend; i++)
        if(vbMatchesInliers[i])
            N++;

    // Compute Essential Matrix from Fundamental Matrix
    cout << "Compute Essential Matrix from Fundamental Matrix" << endl;
    cout << "K.size: " << K.size() << endl;
    cout << "F21.size: " << F21.size() << endl;
    ...
    cout << "begin finding right answer"  << endl;

    //mvMatches12:存放点对匹配
    //vbMatchesInliers:存放的是对于每一个点是不是内点的true or false，从投影点到极线距离得到
    //vP3Dx:存放的是计算出来的三维点信息
    //vbTriangulated:存放的是点是否能够三角化的true or false
    //mSigma2:是初始化initializer类时输入的sigma的平方,在CheckRT函数中表示为th2,即点到极线距离的阈值

    int nGood1 = CheckRT(R1,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D1, 4.0*mSigma2, vbTriangulated1, parallax1);
    int nGood2 = CheckRT(R2,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D2, 4.0*mSigma2, vbTriangulated2, parallax2);
    int nGood3 = CheckRT(R1,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D3, 4.0*mSigma2, vbTriangulated3, parallax3);
    int nGood4 = CheckRT(R2,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D4, 4.0*mSigma2, vbTriangulated4, parallax4);
//    int nGood1 = CheckRT(R1,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D1, 8.0*mSigma2, vbTriangulated1, parallax1);
//    int nGood2 = CheckRT(R2,t1,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D2, 8.0*mSigma2, vbTriangulated2, parallax2);
//    int nGood3 = CheckRT(R1,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D3, 8.0*mSigma2, vbTriangulated3, parallax3);
//    int nGood4 = CheckRT(R2,t2,mvKeys1,mvKeys2,mvMatches12,vbMatchesInliers,K, vP3D4, 8.0*mSigma2, vbTriangulated4, parallax4);
    cout << "finding process done"  << endl;
...
//	cout << "squareError1: " << squareError1 << "--";
...
//	cout << "squareError2: " << squareError2 << "--";
